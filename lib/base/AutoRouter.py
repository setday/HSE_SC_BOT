...
class AutoRouter(Router):
    ...
    @deprecate(message="This is a workaround method, use add edge methods instead to make proper entry points for nodes")
    ...
    def add_button_edge(
        ...
        if not isinstance(next_node, ExtraAutoNodes) and not next_node.startswith("http"):
        ...
        assert button_text is not None, "Button text can't be None for edge with ordinary nodes"
    ...
    def add_message_edge(
        ...
        assert not self.node_dict[node_name].has_next_message_node(), "Message handler already registered"

        self.node_dict[node_name].add_state_changer(self.node_dict[next_node_name].get_node_state())
